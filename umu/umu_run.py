...
def umu_run(args: Namespace | tuple[str, list[str]]) -> int:
    ...
    if not runtime_version:
        ...
        raise ValueError(err)
    # runtime_name, runtime_path, runtime_appid
    _, runtime_path, _ = runtime_version
    os.environ["RUNTIMEPATH"] = runtime_path
    ...
    with (
        ...
        if runtime_path != "host":
            UMU_LOCAL.joinpath(runtime_path).mkdir(parents=True, exist_ok=True)

            future: Future = thread_pool.submit(
                setup_umu, UMU_LOCAL / runtime_path, runtime_version, session_pools
            )
    ...
    # Build the command
    command: tuple[Path | str, ...] = build_command(env, UMU_LOCAL, runtime_path, opts)
    log.debug("%s", command)
